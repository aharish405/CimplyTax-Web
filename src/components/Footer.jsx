...
const cols = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Trademark',            href: '/company-incorporation' },
    ],
  },
  {
    title: 'Loans',
...
export default function Footer() {
  return (
    <footer className="bg-slate-950 border-t border-slate-800" role="contentinfo">
      {/* Main grid */}
      <div className="container-xl py-16 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
          {/* Brand col */}
          <div className="lg:col-span-1">
            <Link to="/" className="flex items-center gap-2 mb-4">
              <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-trust-500 to-navy-700 flex items-center justify-center">
                <span className="text-white font-bold font-display">CT</span>
              </div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-5">
              India's trusted CA-led platform for Tax, GST, and Business compliance services since 2015.
            </p>
            <div className="flex gap-3">
              {socials.map((s) => (
                <a
                  ...
                  className="w-9 h-9 rounded-lg bg-slate-800 text-slate-400 hover:bg-trust-700 hover:text-white flex items-center justify-center transition-all duration-200"
                >
                  {s.icon}
                </a>
              ))}
              ...
              <ul className="space-y-2.5">
                {col.links.map((lnk) => (
                  <li key={lnk.label}>
                    <Link
                      to={lnk.href}
                      className="text-slate-400 hover:text-trust-300 text-sm transition-colors"
                    >
                      {lnk.label}
                    </Link>
                  </li>
      ...
      <div className="border-t border-slate-800">
        <div className="container-xl px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p className="text-slate-600 text-xs">
            Income Tax Return Filing | GST Filing | Company Registration | Loan Services
          </p>
        </div>
      </div>
    </footer>
